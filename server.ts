...
const EVIDENCIA_API_BASE = 'https://api.evidenciacalcados.com.br';
const EVIDENCIA_LOGIN_URL = `${EVIDENCIA_API_BASE}/api/v1/login`;
...
async function getValidToken(forceRefresh = false): Promise<string> {
  ...
  pendingTokenPromise = (async () => {
    console.log('[Backend Auth] Renovando token via login na API da Evidência Calçados...');
    ...
    try {
      ...
      console.log(`[Backend Auth] Token obtido com sucesso via login. Válido até: ${new Date(tokenExpiresAt).toLocaleString('pt-BR')}`);
      return token;
    } catch (err: any) {
      clearTimeout(timeoutId);
...
// Endpoint para fornecer token válido ao frontend para requisições diretas a https://api.evidenciacalcados.com.br
...
app.use(['/api/v1', '/v1'], async (req, res, next) => {
  ...
  try {
    ...
    if (apiRes.status === 401) {
      console.warn(`[Backend Proxy] HTTP 401 em ${subPath}. Forçando renovação imediata do token...`);
    ...
    if (!apiRes.ok) {
      ...
      if (apiRes.status === 404 && subPath.includes('/produtos/grupos')) {
        console.info('[Backend Proxy] Rota remota de grupos indisponível (404). Retornando lista vazia de grupos para geração dinâmica via catálogo.');
        return res.json([]);
      }
...
async function startServer() {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`Servidor Evidência Calçados backend proxy rodando na porta ${PORT}`);
  });
}
