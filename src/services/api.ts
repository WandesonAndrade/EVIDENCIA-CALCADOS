...
const getEnvApiUrl = (): string => {
  const envUrl = typeof import.meta !== 'undefined' && import.meta.env
    ? (import.meta.env.VITE_API_URL || import.meta.env.VITE_MOBLINK_API_URL || import.meta.env.VITE_MOBLINK_API_BASE_URL)
...
export const API_ENDPOINTS = {
  ...
  PRODUTO_SINGLE: (id: string | number) => getApiEndpoint(`/produtos/${String(id).trim()}`),

  /** Rota de variações de grade de um produto: GET ${VITE_API_URL}/produtos/{id}/grades */
  PRODUTO_GRADES: (id: string | number) => getApiEndpoint(`/produtos/${String(id).trim()}/grades`),
  ...
  GRADE_SINGLE: (id: string | number) => getApiEndpoint(`/gradesprodutos/${String(id).trim()}`),
  ...
  CLIENTE_SINGLE: (id: string | number) => getApiEndpoint(`/clientes/${String(id).trim()}`),
  ...
  CLIENTE_CONTAS_RECEBER: (id: string | number) => getApiEndpoint(`/clientes/${String(id).trim()}/contas-receber?formatada=false&vencidas=false`),
