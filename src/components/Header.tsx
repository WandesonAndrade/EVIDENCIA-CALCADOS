...
import { ShoppingBag, Search, User, LogOut, History, ChevronDown, Heart, Sun, Moon, Shield, CreditCard, Truck, RefreshCw, MessageSquare, MapPin } from 'lucide-react';
...
export const Header: React.FC = () => {
  ...
  const navCategories = React.useMemo(() => {
    ...
    defaultNavs.forEach((nav) => {
      if (!seenKeys.has(nav.key)) {
        seenKeys.add(nav.key);
        if (nav.isPromo) {
          uniqueNavs.push(nav);
        } else {
  ...
  return (
    <header id="store-header" className="sticky top-0 z-40 w-full transition-all duration-300">
      ...
      <div className="bg-[#111111] text-white text-[11px] font-medium py-1.5 px-4 sm:px-8 border-b border-neutral-800">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-1 sm:gap-4 text-center sm:text-left">
          ...
          <div className="hidden md:flex items-center space-x-4 text-neutral-300 font-normal">
            <span className="flex items-center space-x-1 hover:text-white transition-colors cursor-pointer" onClick={() => setCurrentView('support')}>
            ...
            <span className="text-neutral-600">|</span>
            <span className="flex items-center space-x-1 hover:text-white transition-colors cursor-pointer" onClick={() => setCurrentView('about')}>
      ...
      <div className={`border-b backdrop-blur-2xl transition-all duration-300 ${
        isDark 
          ? 'bg-[#0B0F19]/95 border-slate-800/80 text-white shadow-xl shadow-black/30' 
          : 'bg-white/95 border-neutral-200/90 text-neutral-900 shadow-xs'
      }`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex items-center space-x-3 sm:space-x-4 shrink-0">
              ...
              <button
                ...
                className={`relative flex items-center space-x-1.5 text-xs font-semibold transition-all cursor-pointer p-2 rounded-full ${
                  currentView === 'favorites'
                    ...
                    : isDark ? 'text-slate-300 hover:text-white hover:bg-slate-800' : 'text-neutral-700 hover:text-black hover:bg-neutral-100'
                }`}
              ...
              <button
                ...
                className={`p-2 rounded-full transition-all cursor-pointer ${
                  isDark ? 'text-amber-400 hover:bg-slate-800' : 'text-neutral-600 hover:bg-neutral-100'
                }`}
                ...
                <div className="relative">
                  <button
                    ...
                    className={`flex items-center space-x-1.5 text-xs font-semibold p-1.5 rounded-full border transition-all cursor-pointer ${
                      isDark ? 'bg-slate-900 border-slate-800' : 'bg-neutral-100 border-neutral-200'
                    }`}
                  >
                    {activeUser.photoURL ? (
                      <img 
                      ...
                      <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${
                        isDark ? 'bg-slate-800 text-amber-400' : 'bg-neutral-200 text-neutral-800'
                      }`}>
                  ...
                  <AnimatePresence>
                    {isDropdownOpen && (
                      <>
                        ...
                        >
                          <div className={`px-4 py-2.5 border-b ${isDark ? 'border-slate-800' : 'border-neutral-100'}`}>
                            <p className="text-xs font-bold truncate">{activeUser.name || 'Usuário'}</p>
                            <p className="text-[10px] text-neutral-400 truncate">{activeUser.email || ''}</p>
                          </div>
                          ...
                          <div className="py-1">
                            <button
                              ...
                              className={`w-full text-left px-4 py-2 text-xs font-medium transition-colors flex items-center space-x-2 cursor-pointer ${
                                isDark ? 'text-slate-300 hover:bg-slate-800' : 'text-neutral-700 hover:bg-neutral-100'
                              }`}
                            >
                            ...
                            {isAuthorizedCollaborator && (
                              <button
                                ...
                                className={`w-full text-left px-4 py-2 text-xs font-bold transition-colors flex items-center space-x-2 cursor-pointer ${
                                  isDark ? 'text-amber-400 hover:bg-slate-800' : 'text-blue-600 hover:bg-neutral-100'
                                }`}
                              >
                          ...
                          <div className={`border-t pt-1 mt-1 ${isDark ? 'border-slate-800' : 'border-neutral-100'}`}>
                            <button
                              onClick={() => {
                                logout();
                                setIsDropdownOpen(false);
          ...
          <div className={`border-t py-2.5 flex items-center justify-center overflow-x-auto no-scrollbar ${
                    ...
                    className={`transition-colors cursor-pointer hover:underline ${
                      item.isPromo 
                        ...
                        : isActive 
                          ...
                          : isDark ? 'text-slate-300 hover:text-white' : 'text-neutral-700 hover:text-black'
                    }`}
                  >
                    {item.name}
                  </button>
