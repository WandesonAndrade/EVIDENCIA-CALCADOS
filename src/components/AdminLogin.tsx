...
export const AdminLogin: React.FC = () => {
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMessage(
        err.message ||
          "Credenciais incorretas ou este perfil não possui privilégios administrativos.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleGoogleAdminLogin = async () => {
    ...
    try {
      ...
      if (userProfile.role === "admin" || userProfile.role === "seller") {
        setCurrentAdminUser(userProfile);
        localStorage.setItem(
          "evidencia_admin_user",
          JSON.stringify(userProfile),
        );
        setCurrentView("admin");
      } else {
        setErrorMessage(
          `Acesso Não Autorizado: O e-mail (${userProfile.email}) não está na lista de colaboradores autorizados. Solicite a liberação ao administrador da loja.`,
        );
      }
    } catch (err: any) {
      console.error(err);
  ...
  const handleQuickLogin = async (testRole: "admin" | "seller") => {
    ...
    try {
      const testEmail =
        testRole === "admin" ? "admin@evidencia.com" : "vendedor@evidencia.com";
      ...
      try {
        ...
        console.warn(
          "📌 Fallback direto de homologação ativado para atalho rápido:",
          err,
        );
        profile = {
          uid:
            testRole === "admin" ? "admin_homolog_uid" : "vendedor_homolog_uid",
          name:
            testRole === "admin"
              ? "Administrador Evidência"
              : "Vendedor Evidência",
          email: testEmail,
          role: testRole,
  ...
  return (
    <div id="admin-auth-page" className="max-w-md mx-auto px-4 py-8 sm:py-14">
      <div
        className={`rounded-3xl border backdrop-blur-2xl shadow-2xl p-6 sm:p-10 space-y-6 ${
          isDark
      ...
      >
        ...
        <div className="text-center space-y-3">
          ...
          <h2
            className={`text-lg sm:text-xl font-black tracking-tight ${isDark ? "text-white" : "text-slate-900"}`}
          >
            Autenticação Unificada da Equipe
          </h2>
          <p
            className={`text-xs max-w-xs mx-auto leading-relaxed ${isDark ? "text-slate-400" : "text-slate-500"}`}
          >
            Entrada direta para colaboradores usando a sua Conta Google
        ...
        <div className="space-y-4 pt-2">
          ...
          >
            ...
            <span>
              {isLoading
                ...
                : "Entrar no Painel com Conta Google"}
            </span>
          </button>

          <div className="relative flex items-center justify-center pt-2">
            <div
              className={`border-t w-full ${isDark ? "border-slate-800" : "border-slate-200"}`}
            ></div>
            <span
          ...
          <form onSubmit={handleLoginSubmit} className="space-y-3.5 pt-2">
            <div className="space-y-1">
              <label
                className={`text-[10px] font-extrabold uppercase tracking-wider block ${isDark ? "text-slate-300" : "text-slate-600"}`}
              >
        ...
        <div className="pt-2 border-t border-slate-800/40 space-y-3">
          <p
            className={`text-[10px] font-bold uppercase tracking-wider text-center ${isDark ? "text-slate-500" : "text-slate-400"}`}
          >
